...
- Unreal 패키지에는 Python GUI, C++ Editor 모듈, NativeAOT DLL / `.lib` 파일이 들어 있습니다.
...
Unreal Python 메뉴와 GUI가 C++ Editor 모듈을 호출하고, C++ 모듈이 NativeAOT DLL을 링크해 빌더를 실행합니다. <br>
...
Unreal NativeAOT DLL / `.lib`는 publish로 갱신됩니다.

```bash
dotnet publish Source.Native/Sheet-Schema-Builder.Native.csproj -c Release
```
...
Unreal 에디터 버튼은 Python에서 `USheetSchemaBuilderEditorLibrary`를 호출하고, C++ Editor 모듈이 `SheetSchemaBuilderNative.dll`의 exported 함수를 호출합니다. <br>
...
Package/
  ...
  Unreal/
    Binaries/ThirdParty/         NativeAOT DLL / LIB / PDB
