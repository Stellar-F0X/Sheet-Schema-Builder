...
| 행 | 내용 | 예 |
|---|---|---|
| 1행 | 각 열의 데이터 타입과 선택적인 키 역할 | `int(pk)`, `int(fk)`, `List<int>(dk)`, `float` |
...
- 지원 타입: `int`, `long`, `float`, `double`, `bool`, `string`, `enum:이름` 및 각 타입의 `List<타입>`
- `List<타입>` 셀은 쉼표(`,`)로 원소를 구분합니다. 예: `5, 81, 552, 4` → JSON 배열 `[5, 81, 552, 4]`
- 빈 리스트 셀은 빈 배열로 저장됩니다. `1,,2`처럼 중간에 빈 원소가 있으면 오류입니다.
...
- `List<자료형>(fk)`/`List<자료형>(dk)`: 각 리스트 원소를 대상 PK로 해석합니다. 생성된 행 구조체의 `Get필드명(database)`로 참조 행 목록을 가져올 수 있습니다.
- PK는 시트마다 0개 또는 1개만 허용됩니다. 2개 이상이면 오류입니다.
- `List<타입>(pk)`는 지원하지 않습니다.
...
- `List<타입>(dk)`는 각 원소별로 동일한 DK 역방향 인덱스를 구성하므로, 대상 PK 행에서도 자신을 포함한 모든 자식 행을 조회할 수 있습니다.
