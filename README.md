...
배포 대상은 `Package/Unity`와 `Package/Unreal` 폴더입니다. <br>
- Unity 패키지에는 에디터에서 직접 호출할 `netstandard2.1` 호환 DLL과 의존성 DLL이 들어 있습니다. 
- Unreal 패키지에는 Python GUI, C++ Editor 모듈, 플랫폼별 NativeAOT 라이브러리가 들어 있습니다.
...
Unreal Python 메뉴와 GUI가 C++ Editor 모듈을 호출하고, C++ 모듈이 플랫폼별 NativeAOT 라이브러리를 로드해 빌더를 실행합니다. <br>
...
또한 `Package/Unity/UnityCodeTemplates.txt`, `Package/Unreal/UnrealCodeTemplate.txt`처럼 DLL 주변의 엔진별 템플릿 파일이 있으면 그 파일을 우선 사용합니다.
...
이 경우 `Package/Unreal/Sheet-Schema-Builder.ini`를 직접 수정한 뒤 Unreal Python에서 `unreal.SheetSchemaBuilderEditorLibrary.run_sheet_schema_builder(...)`를 호출할 수 있습니다.
...
Package/
  ...
  Unreal/
    Binaries/ThirdParty/         Win64(.dll), Linux(.so), Mac(.dylib) NativeAOT 산출물
...
패키지 산출물은 GitHub Actions에서 자동으로 빌드됩니다. 로컬에서 직접 갱신할 때는 Unity 호환 DLL은 일반 빌드, Unreal NativeAOT 라이브러리는 대상 OS별 RID publish를 사용합니다.
...
dotnet publish Source.Native/Sheet-Schema-Builder.Native.csproj -c Release -r linux-x64
dotnet publish Source.Native/Sheet-Schema-Builder.Native.csproj -c Release -r osx-arm64
```

NativeAOT는 OS별 native toolchain을 사용하므로 Linux와 macOS 산출물은 GitHub Actions의 각 OS runner에서 빌드하는 것을 권장합니다.
